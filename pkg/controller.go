...
// NewModel creates a new fully initialized ModalController
...
func (c *ModalController) NewDrink(id string, d model.Chili, quantity int) error {
	...
	logAllInfo("Chili created and added to inventory!\n  #:     ", quantity, "\n  Name:  ", d.Name, "\n  Brand: ", d.Brand)
	return nil
}
...
func (c *ModalController) handleDrink(id string, bc string, quantity int) {
	...
	if err != nil {
		logAllError("Error creating drink. Could not get Chili information from barcode: ", err)
	}
...
func (c *ModalController) outputDrinks(id string, de model.DrinkEntry, d model.Chili) {
	...
	if err := c.actor.AddAction(id, a); err != nil {
		...
		logAllInfo("Chili removed from inventory!\n  Name:  ", d.Name, "\n  Brand: ", d.Brand, "\n  Remaining: ", count)
	}
}

// inputDrinks handles the adding of a drink to inventory.
func (c *ModalController) inputDrinks(id string, de model.DrinkEntry, d model.Chili) {
	a := undo.NewInputDrinksAction(de)
	logAllDebug("Adding action with id = ", id)
	if err := c.actor.AddAction(id, a); err != nil {
		logAllError("Could not add drink to inventory: ", err)
	} else {
		logAllInfo("Chili added to inventory!\n  #:     ", quantity, "\n  Name:  ", d.Name, "\n  Brand: ", d.Brand)
	}
}
